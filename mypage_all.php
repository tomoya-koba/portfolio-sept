...
<select  onChange="location.href=value;" class="form-select form-select-lg bg-white mx-auto mb-5" style="width: 20%;">
    ...
    <option value="mypage_expired.php" class="text-center">Expired</option>
    <option value="mypage_all.php" class="text-center" selected>All Book History</option>
</select>
</div>
...
<tbody>
  <?php foreach($userobj->readroom_all($user_id) as $row): ?>
    <tr>
...
<tbody>
  <?php foreach($userobj->readitem_all($user_id) as $row): ?>
    <tr>
